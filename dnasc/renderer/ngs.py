...
def _well_coord(pos, rows=8) -> str:
    """LIMS `well.position` → plate coordinate. Position is 0-BASED and column-major: position 54
    on a 96-well plate is G7 (row 54%8=6→'G', col 54//8+1=7). The LIMS well DETAIL page shows the
    raw 0-based number; every other view shows position+1."""
    try:
        p = int(float(pos))
